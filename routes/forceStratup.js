...
if (!pass) {
  ...
  for (line in queriesContentsInLines) {
    if (queriesContentsInLines[line].includes("\"name\"")) {
      console.log(queriesContentsInLines[line]);

      var queryname = queriesContentsInLines[line].split("name\":")[1].split(',')[0].replace(/^"|"$/g, '');
  ...
  for (key in files) {
    ...
    if (queryNames.indexOf(fileName) == -1) {
      ...
      addedQueriesContents += '"query" :' + JSON.stringify(currentQueryFileContent) + '\n}\n';
    }
  }
  ...
  var upperFusekiQueriesFileContent = 'define( [], function() {\n' +
    ...
    '        "prefixes": ["owl", "rdfs"]\n'+
    '      }\n';
  console.log(upperFusekiQueriesFileContent + addedQueriesContents);
  // combine the upper upperFusekiQueriesFileContent with the new queries if there is any
  fs.writeFileSync(fusekiQueriesFilePath, upperFusekiQueriesFileContent + addedQueriesContents);
